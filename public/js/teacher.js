...
if (utilisateur) {
  ...
  document
    .querySelectorAll(".puces-menu button[data-panel]")
    .forEach((bouton) => {
      bouton.addEventListener("click", () =>
        panneaux[bouton.dataset.panel](contenu),
      );
    });
...
function filtrerParMatricule() {
  ...
  const tableauEleves =
    document.getElementById("tableau-eleves") ||
...
async function chargerProfil() {
  try {
    ...
    document.getElementById("nomProfesseur").textContent =
      `${professeur.prenom} ${professeur.nom}`;
    document.getElementById("nomProfesseurCarte").textContent =
      `${professeur.prenom} ${professeur.nom}`;
  } catch (erreur) {
