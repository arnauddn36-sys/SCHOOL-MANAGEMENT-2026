...
function afficherListe(conteneur, notes) {
  ...
  conteneur
    .querySelector("#boutonAjouterNote")
    .addEventListener("click", () => afficherFormulaireAjout(conteneur));

  conteneur.querySelectorAll(".modifier-note").forEach((bouton) => {
    bouton.addEventListener("click", () =>
      afficherFormulaireModification(conteneur, bouton.dataset.id),
    );
  });

  conteneur.querySelectorAll(".supprimer-note").forEach((bouton) => {
    bouton.addEventListener("click", () =>
      gererSuppression(conteneur, bouton.dataset.id),
    );
  });
}
        ...
        <tr>
            ...
            <td>
                <div class="actions-ligne">
                    <button class="secondaire modifier-note" data-id="${note.id}">Modifier</button>
...
async function afficherFormulaireAjout(conteneur) {
  try {
    ...
    conteneur
      .querySelector("#boutonAnnulerNote")
      .addEventListener("click", () => afficherPanneauNotes(conteneur));

    conteneur
      .querySelector("#formulaireNote")
      .addEventListener("submit", (evenement) =>
        gererEnvoiAjout(evenement, conteneur),
      );
  } catch (erreur) {
    afficherNotification(
      "Impossible de charger les élèves ou les matières",
      "error",
    );
  }
}
...
async function afficherFormulaireModification(conteneur, id) {
  try {
    // On récupère toutes les notes, les élèves et les matières en parallèle
    ...
    const noteCourante = notes.find((n) => String(n.id) === String(id));
    if (!noteCourante) {
      afficherNotification("Note introuvable", "error");
      return;
    }
    ...
    conteneur.innerHTML = `
            ...
            <form id="formulaireModificationNote">
                <label for="noteEleve">Élève</label>
                <select id="noteEleve">
                    ${eleves
                      .map(
                        (eleve) =>
                          `<option value="${eleve.id}" ${eleve.id === noteCourante.student_id ? "selected" : ""}>${echapperHtml(eleve.nom)} ${echapperHtml(eleve.prenom)}</option>`,
                      )
                      .join("")}
                </select>
                ...
                <select id="noteMatiere">
                    ${matieres
                      .map(
                        (matiere) =>
                          `<option value="${matiere.id}" ${matiere.id === noteCourante.subject_id ? "selected" : ""}>${echapperHtml(matiere.nom)}</option>`,
                      )
                      .join("")}
                </select>

                <label for="noteValeur">Note (sur 20)</label>
                <input type="number" id="noteValeur" min="0" max="20" step="0.5" value="${noteCourante.note}" required>
    ...
    conteneur
      .querySelector("#formulaireModificationNote")
      .addEventListener("submit", (evenement) =>
        gererEnvoiModification(evenement, conteneur, id),
      );
  } catch (erreur) {
    afficherNotification(
...
// Envoie les modifications d'une note au serveur
async function gererEnvoiModification(evenement, conteneur, id) {
  ...
  try {
    const resultat = await api.put(`/api/grades/${id}`, donnees);
    afficherNotification(resultat.message || "Note modifiée avec succès");
    await afficherPanneauNotes(conteneur);
  } catch (erreur) {
    afficherNotification(erreur.message, "error");
  }
